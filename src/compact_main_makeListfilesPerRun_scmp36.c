...
/*       } */
    ...
    if(strcmp(argv[i],"-rootfilename") ==0)    /* Set root histogramfile name */
      {	
	strcpy(rootfilename,argv[++i]);
	printf("=======> Set Root Histogramfilename to %s.root\n",rootfilename);
        continue;
      }	
    if(strcmp(argv[i],"-datfilepath") ==0)    /* Set path for datfiles */
      {	
	strcpy(datfilepath,argv[++i]);
	printf("=======> Set datfile path to %s\n",datfilepath);
        continue;
      }	
    if(strcmp(argv[i],"-jobindex") ==0)    /* Set index for single job, valid for outfiles etc. */
      {	
	strcpy(jobindex,argv[++i]);
	printf("=======> Set job index to %s\n",jobindex);
        continue;
      }	
...
/*   } */
  ...
  strcpy(compactFilename,current->fname);
  printf ("\n###### Current file name = %s\n", compactFilename);
...
/*                   if(COMBflag) sbur.dataset=1;       /\* Save info that data are combined *\/ */
		...
		if(cmpReader_options.abcog) { /*need alpha, beta and COG constants*/
	      ...
	      ////////////////////////////////////////////////
	      // v65: make list files per run for scmp data //
	      ////////////////////////////////////////////////

	      printf ("####### IN COMPACT MAIN: run number = %i #######\n", sbur.nrun);
	      
	      // This is the first burst of the current job
	      if (burstRunCounter == 0)
		{
		  sprintf (runListFileName, "%s/run%i.list", datfilepath, sbur.nrun);
		  
		  if ((runListFile = fopen (runListFileName, "w")) == NULL)
		    {
	      ...
	      // A new run has started, this is the first burst
	      if (runIndex < sbur.nrun)
		{
	      ...
	      fprintf (runListFile, "%s\n", compactFilename);
	      
	      // Now skip the whole file, I only want to make the scmp list files for single runs
	      // --> no data analysis !!!
	      JumpToNextFile=1; 
	      break;
	      ////////////////////////////////////////////////
	      // End: make list files per run for scmp data //
	      ////////////////////////////////////////////////
